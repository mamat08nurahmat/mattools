...
select count(a.kolom5) from mismer_bulk a -- 391404
...
-- 728688
select count(*) from mismer_detail -- 382997
select count(*) from mismer_unmatch -- 8407

select count(*) from temp_upload_unmatch
...
CALL clean_match(2018,10); -- 23315 ALL -- 179 is_update=1 & 23136 is_update=0 cleansing
CALL generate_match(2018,10); -- 14664 new bulk
-- select count(*) from mismer_unmatch where BatchID=201810; -- 181
CALL clean_unmatch(2018,10);
CALL generate_unmatch(2018,10);

select count(*) from mismer_detail

select count(*) from mismer_unmatch

select * from temp_upload_unmatch
select count(*) from mismer_detail where is_update=1
select count(*) from mismer_unmatch where is_update=1
...
-- sample export 
 ...
 SELECT 'RowID','BatchID','OPEN_DATE','MID','MERCHAN_DBA_NAME' 
UNION ALL
 SELECT RowID,BatchID,OPEN_DATE,MID,MERCHAN_DBA_NAME
FROM mismer_detail
LIMIT 100
-- INTO OUTFILE '/var/lib/mysql-files/orders.csv'
INTO OUTFILE 'C:/MISMER/mismer123.csv'
...
SELECT temp_upload_unmatch.* FROM temp_upload_unmatch,mismer_unmatch
                      WHERE temp_upload_unmatch.MID = mismer_unmatch.MID
                      
                      
UPDATE mismer_unmatch AS mu
INNER JOIN temp_upload_unmatc AS temp ON mu.MID = temp.MID
SET mu.OPEN_DATE = temp.OPEN_DATE, 
mu.MERCHAN_DBA_NAME = temp.MERCHAN_DBA_NAME,
mu.WILAYAH = temp.WILAYAH,
  mu.CHANNEL = temp.CHANNEL,
  mu.updated_at = temp.created_at
 
-- select * from mismer_unmatch limit 1